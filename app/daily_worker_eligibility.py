...
def daily_worker_eligibility_app():
    ...
    html += (
        ...
        "  document.getElementById('judgment').innerHTML = `<p>${condition1Text}</p><p>${condition2Text}</p>` + (nextPossible1 ? `<p>${nextPossible1}</p>` : "") + (nextPossible2 ? `<p>${nextPossible2}</p>` : "");\n" +
        "  document.getElementById('final').innerHTML = `<p>✅ 일반일용근로자: ${generalWorkerText}</p><p>✅ 건설일용근로자: ${constructionWorkerText}</p>`;\n" +
        "}\n" +
