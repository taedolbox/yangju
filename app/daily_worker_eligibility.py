...
def daily_worker_eligibility_app():
    ...
    function calculateAndDisplayResult(selected) {
        ...
        const generalWorkerText = workedDays < threshold ? "✅ 일반일용근로자 신청 가능" : "❌ 신청 불가능";
        const constructionWorkerText = (workedDays < threshold || noWork14Days) ? "✅ 건설일용근로자 신청 가능" : "❌ 신청 불가능";

        const finalHtml = `
            <h3>📌 조건 기준</h3>
            <p>조건 1: 신청일이 속한 달의 직전 달 첫날부터 신청일까지 근무일 수가 전체 기간의 1/3 미만</p>
            <p>조건 2: 건설일용근로자만 해당, 신청일 직전 14일간 무근무</p>
