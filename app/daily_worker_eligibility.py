...
# HTML for calendar and report
calendar_html = """
<div id="calendar-container">
    <div style="text-align: right; margin-bottom: 15px;">
        <button onclick="clearCalendar()" style="background-color: #3F51B5; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; box-shadow: 0 2px 5px rgba(0,0,0,0.2); transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#303F9F'" onmouseout="this.style.backgroundColor='#3F51B5'">🔄 달력 초기화</button>
    </div>
"""
...
for ym, dates in calendar_groups.items():
    ...
    calendar_html += """
    <div class="calendar">
        <div class="day-header sunday">일</div><div class="day-header">월</div><div class="day-header">화</div><div class="day-header">수</div><div class="day-header">목</div><div class="day-header">금</div><div class="day-header saturday">토</div>
    """
...
<script>
    ...
    function getDaysBetween(startDate, endDate) {
        ...
        if (start > end) return 0; let count = 0; let current = new Date(start);
    ...
    function calculateAndDisplayResult(selectedMMDD) {
        ...
        if (selectedFullDates.length === 0) {
            document.getElementById('resultContainer').innerHTML = `
                <h3>📌 조건 판단</h3><p>✅ 조건 1 충족: 근무일 0일 (선택 없음)</p><p>✅ 조건 2 충족: 근무일 0일 (선택 없음)</p>
                <h3>📌 최종 판단</h3><p>✅ 일반일용근로자: 신청 가능</p><p>✅ 건설일용근로자: 신청 가능</p>
                <h3>📌 종합 신청 가능일</h3><p>근무일이 없으므로, 현재(${INPUT_DATE_STR}) 바로 신청 가능합니다.</p><p>※ 이후 근로제공이 없다는 전제 하에 계산된 것입니다.</p>`;
            return;
        }
        ...
        if (selectedFullDates.includes(INPUT_DATE_STR)) {
            ...
            document.getElementById('resultContainer').innerHTML = `
                <h3 style="color: red;">📌 조건 판단</h3><p style="color: red;">❌ 조건 1 불충족: 기준 날짜(${INPUT_DATE_STR}) 근무로 인한 미충족</p><p style="color: red;">❌ 조건 2 불충족: 기준 날짜(${INPUT_DATE_STR}) 근무로 인한 미충족</p>
                <h3 style="color: red;">📌 최종 판단</h3><p style="color: red;">❌ 일반일용근로자: 신청 불가능</p><p style="color: red;">❌ 건설일용근로자: 신청 불가능</p>
                <h3>📌 종합 신청 가능일</h3><p style="color: red;">${formatDateToYYYYMMDD(nextDate)} 이후 신청 가능</p><p>※ 이후 근로제공이 없다는 전제 하에 계산된 것입니다.</p>`;
            return;
        }

        const currentPeriodStart = (new Date(inputDate)).setMonth(inputDate.getMonth() - 1, 1);
        ...
        const condition2Text = noWork14Days ? `✅ 조건 2 충족: ${FOURTEEN_DAYS_START_STR} ~ ${FOURTEEN_DAYS_END_STR} 무근무` : `❌ 조건 2 불충족: ${FOURTEEN_DAYS_START_STR} ~ ${FOURTEEN_DAYS_END_STR} 내 근무기록`;
        ...
        document.getElementById('resultContainer').innerHTML = `
            ...
            <p>※ 이후 근로제공이 없다는 전제 하에 계산된 것입니다.</p>`;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        stored.forEach(mmdd => document.querySelector(`.day[data-date="${mmdd}"]`)?.classList.add('selected'));
