...
def daily_worker_eligibility_app():
    ...
    # 테스트를 위해 임시로 달력 표시 기간을 줄일 수 있습니다 (예: input_date 이전 30일만 표시)
    # original: first_day_prev_month = (input_date.replace(day=1) - timedelta(days=1)).replace(day=1)
    # original: cal_dates = []
    # original: current_date_for_cal = first_day_prev_month
    # original: while current_date_for_cal <= input_date:
    # original:    cal_dates.append(current_date_for_cal)
    # original:    current_date_for_cal += timedelta(days=1)
    ...
    st.components.v1.html(
        ...
        <script>
            document.getElementById('printReportBtn').onclick = function() {
                console.log("확인서 출력 버튼 클릭됨!"); // 디버깅 메시지
